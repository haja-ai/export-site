...
export async function supabaseRequest(path, options = {}) {
  ...
  const response = await fetch(`${SUPABASE_URL}/rest/v1/${path}`, {
    ...options,
    headers: {
      // Supabase's current secret keys authenticate with the apikey header.
      // Do not place an sb_secret_* key in Authorization: Bearer, which is
      // reserved for user JWTs by the Data API.
...
export async function uploadInquiryAttachment(path, bytes, contentType) {
  requireConfig();
  const response = await fetch(`${SUPABASE_URL}/storage/v1/object/inquiry-attachments/${path}`, {
    method: 'POST',
    headers: {
      apikey: SUPABASE_SERVICE_ROLE_KEY,
