...
/**
 * Send a manually written reply from the official MiniElephant domain.
 * The recipient is never used as the From address; this preserves SPF/DKIM alignment.
 */
export async function sendOfficialReply({ to, subject, message, attachments = [] }) {
  const resendApiKey = process.env.RESEND_API_KEY;
  const fromEmail = process.env.FROM_EMAIL || 'contact@semwheelchair.com';
  const replyTo = process.env.REPLY_TO_EMAIL || process.env.NOTIFICATION_EMAIL || 'johnson@semwheelchair.com';

  if (!resendApiKey) return { success: false, method: 'missing_resend_key' };
...
<html><body style="font-family:Arial,sans-serif;color:#333;line-height:1.7">
  <div style="max-width:640px;margin:0 auto;padding:24px">
    <div style="border-bottom:3px solid #3AB54A;padding-bottom:14px;margin-bottom:20px">
      <strong style="font-size:20px;color:#1a2e3c">MiniElephant B2B</strong>
    </div>
    <div>${escapeHtml(safeMessage).replace(/\\n/g, '<br>')}</div>
    <div style="margin-top:28px;padding-top:14px;border-top:1px solid #e5e7eb;color:#6b7280;font-size:12px">
      MiniElephant Electric Wheelchair<br>
      Jiaxing Small Elephant Medical Technology Co., Ltd<br>
      WhatsApp: +86 13819098967 · johnson@semwheelchair.com<br>
      https://www.semwheelchair.com
    </div>
  </div>
</body></html>`;
  ...
  if (!res.ok) {
    const errorData = await res.text();
    console.error('Resend official reply error:', errorData);
    return { success: false, method: 'resend_error' };
  }
  return { success: true, method: 'resend' };
}
